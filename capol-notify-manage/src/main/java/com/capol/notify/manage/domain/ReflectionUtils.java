...
public class ReflectionUtils {
     ...
     * 		        无属性返回false
     */
    public static boolean hasFieldValue(final Object obj, final String fieldName){
        Field field = getAccessibleField(obj, fieldName);
        if (field == null) {
            return false;
        }
        Object value = ReflectionUtils.getFieldValue(obj, fieldName);
        //改为只要值为空才会填充默认值，否则按开发设置的值进行赋值
    ...
    public static <T> boolean hasFieldValueAsync(final Object obj, final String fieldName) {
        Field field = getAccessibleField(obj, fieldName);
        if (field == null) {
            return false;
        }
